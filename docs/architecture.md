...
**The contract.** The client-facing prose is one artifact, not strings scattered through the adapter. Server instructions, the `consult` tool description, and its five field descriptions all live in one sectioned markdown file (`prompts/contract.md`) whose heading levels mirror MCP's object model: `#` server, `##` tools, `###` the `consult` tool, `####` its description and fields, `#####` each field. `prompts/parser.py` splits the file into a title-keyed mapping (bodies kept byte-verbatim, no markdown AST to reflow what the model reads), and `adapter/_contract.py` validates that mapping into a typed `ServerContract`: one server, one `consult` tool, exactly five fields, with `extra="forbid"` so a drifted or malformed contract fails loud at `create_server`. The adapter reads `contract.instructions` and `contract.tools.consult.description` by attribute, so a heading typo is a pyright error, not a runtime `KeyError`.
