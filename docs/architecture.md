...
3. **Assembly**: `create_server(settings=..., system=partial(system, settings, pool_cell=pool_cell), health_probe=partial(_check_ready, pool_cell), storage=...)` returns the FastMCP instance. `system` is handed over as a factory, not a CM instance: FastMCP's lifespan is ref-counted and re-enterable (the in-memory transport cycles it per client session), so each cycle opens a fresh `system()` scope. `storage` is one bare repository-backed `LoreCacheStore`; the adapter assigns it to fastmcp's two storage slots, session state verbatim and the OAuth lane Fernet-wrapped, since the key material is adapter-owned (see Authentication). Collections keep the lanes isolated; the repository layer never sees OIDC bytes.
...
4. **Pool**: `connect(settings)` opens the `RepositoryPool`; the scope fills the `PoolCell` with the pool.
...
6. **Teardown**: the `finally` arm clears the cell and closes the pool on any exit, including caller exceptions, so `/ready` never vouches for a dead pool, cache storage never reaches one, and connections never leak.

**PoolCell.** The factory must hand `create_server` its repository-backed capabilities (the `health_probe`, the cache store) before any pool exists, so `PoolCell` (in `lore.repositories`) is a deliberately mutable holder tying them all to the pool lifetime: a `RepositoryPool` reference the lifespan fills after `connect()` and clears on exit. The readiness probe (`_check_ready`) raises `StorageError` (the `/ready` 503 shape) whenever no lifespan cycle is live; the cache store treats an empty cell as an internal invariant violation and raises, since OAuth flows and session state only run once the lifespan is live. Probe, store, and orchestrator close over the same pool, filled and cleared inside the same scope. See Authentication for what the storage holds.
...
**The contract.** The client-facing prose is one artifact, not strings scattered through the adapter. Server instructions, the `consult` tool description with its five field descriptions, and the `observe` tool description all live in one sectioned markdown file (`prompts/contract.md`) whose heading levels mirror MCP's object model: `#` server, `##` tools, `###` each tool, `####` a tool's description and fields, `#####` each field. `prompts/_parser.py` splits the file into a title-keyed mapping (bodies kept byte-verbatim, no markdown AST to reflow what the model reads), and `adapter/_contract.py` validates that mapping into a typed `ServerContract`: one server, a `consult` tool with exactly five fields, and an `observe` tool with a description alone, with `extra="forbid"` so a drifted or malformed contract fails loud at `create_server`. The adapter reads `contract.instructions`, `contract.tools.consult.description`, and `contract.tools.observe.description` by attribute, so a heading typo is a pyright error, not a runtime `KeyError`.

**The persona prompt.** The adapter also serves the Scribe persona (`prompts/scribe.md`) as an MCP prompt, deliberately sharing the `consult` name. A user fires it as a slash command (`/mcp__lore__consult`), landing the persona as a real conversation message: the strongest injection channel, where the ambient server `instructions` are the weakest. It takes no required arguments, so the bare invocation works.
...
**Error posture.** `create_server` sets `mask_error_details=True` on the FastMCP instance: hardcoded posture, deliberately overriding the `FASTMCP_*` env default, pinned by a leak test. Any unhandled exception in a tool scrubs to FastMCP's uniform message on the wire; on that path fastmcp itself logs the full cause (`logger.exception` under `fastmcp.server.server`) before masking, so operators keep the diagnostic the client never sees, with no extra middleware. fastmcp's one unmasked arm re-raises a raw pydantic `ValidationError`, echoing the failing value; the orchestrator starves that arm by wrapping any internal `ValidationError` in `DomainInvariantError`: a model construction failure past request validation is a lore bug, never client fault, and the layer that builds the models owns the guarantee. Two deliberate `ToolError` arms do reach the client: the client-fault arm surfaces the violated domain rule verbatim (the constant the domain validator wrote for the Scribe, never pydantic's repr, which would echo the client's payload), and `OracleIdentityMiddleware` rejects an access token without a usable `sub` claim with one constant message (see Authentication). fastmcp binds the flat signature `Field` constraints before the body runs and echoes the client's own out-of-range input as guidance: designed client-fault behavior, not a leak. Correlation ids are ledger and trace identity (the OTel `trace_id`, uuid4 fallback when no span records) and are never client-facing.
...
**Records mirror the schema and double as the orchestrator-visible shape.** Repositories own Pydantic models that mirror the database schema. `HypothesisRecord` is `id`, `content`, `created_at`. `AttestationRecord` is the ten-field ledger row including trust assessment (`t_oracle`), raw and discounted confidence (`c_oracle_raw`, `c_oracle_discounted`), herd snapshot (`c_herd`), and the write-time prior-attester count (`n_oracle_prior`). Each record validates on construction so corrupt data fails loudly; hot-path reads use `model_construct()` to skip validation since the database already enforces the same constraints. There is no separate "domain entity" layer: the orchestrator consumes records directly, and the read-time epistemic projection lives on `SearchResult` (in `lore.domain`), which carries the hypothesis fields it surfaces alongside `c_herd`, `oracle_count`, `last_attested`, and the retrieval scores.
...
**No FastMCP wrapping.** `fastmcp.settings` is already a validated, env-driven model; mirroring it into `lore.toml` would give the same knob two authoritative sources, which this design forbids. The boundary: Lore's TOML owns Lore's identity and behavior (server name, icon, the MCP contract prose, the OIDC DSN with its cross-section invariant); `FASTMCP_*` env owns deployment topology (transport, host, port, banner, update check, statelessness). One exception is hardcoded rather than env-driven: `mask_error_details=True` is posture, pinned by the leak test (see Adapter). Topology defaults live where timing is guaranteed: the Dockerfile ENV block (image) and mise `[env]` (repo), both operator-overridable; under `fastmcp run`, the fastmcp settings singleton snapshots the environment at `import fastmcp`, before any Lore code loads. The log posture is the timing exception: `python -m lore` owns process start, so `main()` setdefaults `FASTMCP_LOG_ENABLED=false` before the first fastmcp import (see Telemetry).
...
**Third-party log routing.** Framework switches, not hijacked loggers. `FASTMCP_LOG_ENABLED=false` is fastmcp's own "I have logging infrastructure" switch: it never installs its Rich handler, and its records propagate to the root structlog handler natively. Whoever owns process start owns the posture: `python -m lore` setdefaults the switch in code before the first fastmcp import (fastmcp snapshots env into its settings singleton at import), and `fastmcp run` paths get it from launcher env (mise `[env]` for the repo, `.env.claude` for installed clients). A bare `fastmcp run` outside those speaks the framework's own Rich console: the framework runner, the framework posture, not a bug. The image entry point also passes `uvicorn_config={"log_config": None}` on non-stdio transports, so uvicorn's records propagate into the JSON pipeline instead of its plaintext handlers. LiteLLM ships no off-switch: `configure_telemetry()` resets its three loggers to bare stdlib defaults, the one documented reroute.
...
When `OIDC_URL` is configured, the adapter runs the standard MCP OAuth flow with that IdP. `OracleIdentityMiddleware` resolves the oracle identity on every tool call: the token's `sub` claim when authenticated, the synthetic `_local` otherwise (appropriate for stdio dev and for HTTP topologies that authenticate at an upstream proxy), stashed in request state so no tool body touches tokens. Oracle identity is a plain string on the ledger; there is no dedicated table and no auto-creation step. No layer below the adapter knows about OAuth; the orchestrator receives a verified oracle identity string, not a token.

The `_*` namespace is reserved for the synthetic identities (`_local`, `_transfer`). The middleware rejects a token whose `sub` is missing, empty, or starts with `_`, with one constant message, so no IdP-claimed name can impersonate a synthetic; each rejection logs `auth.rejected` (tool, offending `sub`) at WARNING server-side, while the wire carries only the constant. The Recorder additionally refuses `_transfer` as a principal at the domain layer: defense-in-depth for the one name written with full credibility.

**Auth opt-in.** `[auth] required` is the operator-controlled fail-fast. Default `false`: Lore reads the FastMCP env-var contract (`FASTMCP_HOST`, `FASTMCP_TRANSPORT`) and never second-guesses it. Operators who want bootstrap to refuse without OIDC set `required = true` under `[auth]` in `lore.toml`; the missing `OIDC_URL` is then a hard error at startup, raised during settings load before migrations or the server start. The auth↔OIDC pairing is a cross-section invariant on `LoreSettings`, validated as the settings model is built rather than as a separate procedural step. The startup sequence is factory (telemetry → settings, where cross-section invariants including auth↔OIDC are validated) → lifespan (migrations → repos → providers → orchestrator); the refusal fires in the factory before any I/O opens, surfacing as a `ConfigurationError`. Under `python -m lore` the entry point logs it as one `bootstrap.refused` ERROR and exits nonzero (telemetry comes first, so the event routes through structlog instead of an interpreter traceback); under `fastmcp run` the framework surfaces it.
...
**OAuth state at rest.** When Lore runs OIDC itself, fastmcp's `OAuthProxy` needs storage for OAuth client registrations and upstream tokens. Its default is an encrypted store on local disk, which neither survives an ephemeral container restart nor spans replicas: every redeploy logs every oracle out. Lore instead injects `client_storage` backed by the repository layer: `CacheRepository` over the `_cache` table (both backends, underscore-prefixed like `_system` because it is infrastructure, not domain), a keyed upsert cache deliberately outside the Ledger's append-only invariant, since token refresh churns the same key and this is operational state, not epistemic evidence. Values are Fernet-encrypted before they reach the database; the adapter owns the wrapping because it owns the key material, so the OIDC client secret never leaves the module that already unwraps it for `OIDCProxy`. The key derives (PBKDF2) from that secret, salted with the OIDC client id: public (salts need no secrecy, only unpredictability before theft), per-deployment (precomputed tables buy nothing), replica-stable, and rotating only alongside the secret. A raw row read yields ciphertext. An undecryptable row (rotated secret, corrupted blob) reads as a cache miss and forces re-auth rather than crashing. This is what makes the Postgres topology safe for restarts, redeploys, and multiple replicas: sessions live wherever the ledger lives. The plumbing is `PoolCell` (see Bootstrap): auth is built before the pool exists, and the lifespan fills the cell once `connect()` succeeds.

**Session state at rest.** The same table serves fastmcp's `session_state_store` (`ctx.get_state`/`set_state`), which otherwise defaults to process memory. The store is injected unconditionally: session state persists in every topology, OIDC or not. It is deliberately unencrypted, unlike OAuth state: operational app state in the same trust domain as the domain tables, not credentials, and a key derived from the OIDC client secret would couple session persistence to one topology and drop all sessions on a routine secret rotation. fastmcp stamps every session-state put with a 24-hour TTL; expired rows read as misses immediately.

**The sweep.** Expired `_cache` rows are deleted by a lifespan-owned task running `repositories.sweep_cache_loop`: once right after `connect()` (restart-heavy deployments boot often) and every `[cache] sweep_interval` thereafter (default hourly). The mechanism lives with the cache store; the composition root owns only the task lifecycle. Rows with no expiry, OAuth client registrations, are exempt by construction: persisting them is the feature. On PostgreSQL the sweep takes a transaction-scoped advisory try-lock (`lore_cache_sweep`, the `lore_migrations` precedent), so replicas on the same schedule don't scan redundantly; the lock releases at COMMIT and cannot leak on a pooled connection. SQLite needs no counterpart: the pool's lock already serializes writers. A failed sweep logs `cache.sweep.failed` and waits for the next tick; it never takes down the lifespan. The sweep scans through a partial index on `expires_at` that excludes NULL-expiry rows by construction; the composite primary key `(collection, key)` covers every other query the repository issues.
