...
The `_*` namespace is reserved for the synthetic identities (`_local`, `_transfer`). The middleware rejects a token whose `sub` is missing, empty, or starts with `_`, with one constant message, so no IdP-claimed name can impersonate a synthetic. The Recorder additionally refuses `_transfer` as a principal at the domain layer: defense-in-depth for the one name written with full credibility.
